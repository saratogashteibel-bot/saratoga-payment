module.exports = async function handler(req, res) {
  ...
  const { type, cardToken, cvvToken, exp, fname, lname, email, street, city, state, zip, amount } = req.body;
  ...
  try {
    if (type === 'donation') {
      ...
      if (scheduleResult.Result !== 'S') {
        return res.json({ success: true, warning: 'First payment processed but recurring schedule could not be set up.' });
      }
